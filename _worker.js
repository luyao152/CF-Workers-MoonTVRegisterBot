...
async function handleCheckEndpoint(apiUrl, username, password, KV) {
  ...
  checkResult.summary = {
    ...
    overallStatus: checkResult.moontvApi.status === 'connected' && checkResult.cookieStatus.valid && checkResult.configApi.accessible ? 'healthy' : 'unhealthy'
  };
...
async function handleTelegramWebhook(request, bot_token, GROUP_ID, apiUrl, moontvUrl, username, password, KV, siteName) {
  try {
    ...
    if (update.message && update.message.text) {
      ...
      // 检查是否是密码输入状态
      const passwordState = await KV.get(`password_state:${userId}`);
      if (passwordState === 'waiting') {
        return await handlePasswordInput(bot_token, userId, chatId, message.chat.type, GROUP_ID, text, apiUrl, moontvUrl, username, password, KV, siteName);
      }
      ...
      // 处理 /pwd 命令
      if (normalizedText === '/pwd') {
        return await handlePasswordCommand(bot_token, userId, chatId, message.chat.type, GROUP_ID, apiUrl, moontvUrl, username, password, KV, siteName);
      }
...
📊 核心统计
👥 总用户数: ${userCount} 人
🎞️ 视 频 源: ${activeSourceCount}/${sourceCount} 个
📺 直 播 源: ${activeLiveCount}/${liveCount} 个

⚙️ 系统信息
🔄 配置更新: ${lastUpdateTime}
🎯 自动更新: ${configResult.Config.ConfigSubscribtion?.AutoUpdate ? '✅ 已启用' : '❌ 已禁用'}
🕐 缓存时间: ${configResult.Config.SiteConfig?.SiteInterfaceCacheTime || 7200} 秒
🔍 搜索页数: 最大 ${configResult.Config.SiteConfig?.SearchDownstreamMaxPage || 5} 页
...
📈 服务质量
⚡ 状态: ${getLatencyStatus(apiResponseTime)} ${apiResponseTime}ms
🌐 访问: ${getLatencyStatus(moontvResponseTime)} ${moontvResponseTime !== null ? moontvResponseTime + 'ms' : '未知'}
📱 移动端: 兼容
...
// 处理 /pwd 命令 - 请求用户输入新密码
async function handlePasswordCommand(bot_token, userId, chatId, chatType, GROUP_ID, apiUrl, moontvUrl, username, password, KV, siteName) {
  try {
    // 检查是否在群聊或超级群组中
    if (chatType === 'group' || chatType === 'supergroup') {
      // 在群聊中，只提示用户私聊机器人
    ...
    // 设置密码输入状态，有效期10分钟
    await KV.put(`password_state:${userId}`, 'waiting', { expirationTtl: 600 });
...
// 处理密码输入
async function handlePasswordInput(bot_token, userId, chatId, chatType, GROUP_ID, newPassword, apiUrl, moontvUrl, username, password, KV, siteName) {
  try {
    // 清除密码输入状态
    await KV.delete(`password_state:${userId}`);

    // 验证密码长度
    if (!newPassword || newPassword.length < 6 || newPassword.length > 12) {
      await sendMessage(bot_token, chatId, 
        "❌ <b>密码格式错误</b>\n\n" +
        "密码长度必须在6-12位之间，请重新使用 <code>/pwd</code> 命令。\n\n" +
        "💡 示例：<code>123456</code> 或 <code>mypassword123</code>", 
        moontvUrl, siteName
      );
      return new Response('OK');
    }
...
async function sendMessage(bot_token, chatId, text, moontvUrl = null, siteName = null, appInfo = null) {
  try {
    ...
    if (moontvUrl && siteName) {
      ...
      inlineKeyboard.push([{ text: buttonText, url: moontvUrl }]);
    }

    // 如果提供了 appInfo，添加APP下载按钮
    if (appInfo && appInfo.downloadUrl && appInfo.version) {
      const appButtonText = `📱 APP客户端下载 ${appInfo.version}`;
      inlineKeyboard.push([{ text: appButtonText, url: appInfo.downloadUrl }]);
    } else {
      // 添加默认的APP下载按钮
      const defaultAppButtonText = '📱 APP客户端下载';
      inlineKeyboard.push([{ text: defaultAppButtonText, url: 'https://github.com/MoonTechLab/Selene/releases' }]);
    }
